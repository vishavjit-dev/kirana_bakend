...
const editBanner = async (req, res) => {
  try {
    ...
    // Retrieve the existing banner data
    ...
    // Update banner data, preserving existing images if no new files are uploaded
    const updateBanners = {
      bannerTitle,
      bannerLink,
      sliderBannerImage: newDocumentPic.length > 0 ? newDocumentPic : existingBanner.sliderBannerImage, // Use new images if available, otherwise keep existing ones
    };

    // Update the banner in the database
    const updatedBanner = await banner.findByIdAndUpdate(id, updateBanners, { new: true });
    if (!updatedBanner) {
      return res.status(404).json({ message: "Banner not found" });
    }
